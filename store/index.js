export const state = () => ({
  ...
  export const actions = {
    async nuxtServerInit({ commit }, ){
      try {
      const afterAuthorization = await this.$axios.$post(`${this.state.BASE_URL}login`, {
        "login": "9614615230",
        "password": "1333886511",
        "fromuser": 0
      }, { headers: {'Content-Type': 'application/json', 'Accept': 'application/json', 'X-Api-Key': `${this.state.API_KEY}`}})
      commit('setTokenAndId', afterAuthorization)
      const userData = await this.$axios.$get(`${this.state.BASE_URL}user/${afterAuthorization.user.id}`, 
      { headers: {'Accept': 'application/json', 'X-Api-Key': `${this.state.API_KEY}`, 'X-User-Token': `${afterAuthorization.token}`}})
    ...
    async handlerChangeEmail({ commit }, email) {
      try{
        const changeResponse = await this.$axios.$put(`${this.state.BASE_URL}user/${this.state.userId}`, {
          "email": email
        },
        { headers: {'Accept': 'application/json', 'X-Api-Key': `${this.state.API_KEY}`, 'X-User-Token': `${this.state.userToken}`}})
      } catch(e){
        console.log(e)
      }
    }
